...
import com.wangtiansoft.KingDarts.persistence.entity.UserInfo;
...
public class UserAPIController extends BaseController {
    ...
    public @ResponseBody
    ApiResult login(final String code, final String type) {
     ...
     * 爱镖娱乐调用接口
     */
    @RequestMapping("/ABlogin")
    public @ResponseBody
    ApiResult ABlogin(final String unionid) {
        ApiResult result = this.buildAjaxResponse(new IWebResponseHandler() {
            @Override
            public Object execute() throws Exception {
                if (StringUtils.isEmpty(unionid)) {
                    throw new AppRuntimeException(Constants.kCode_Null, "unionid is null");
                }
...
//					userService.addOrUpdateUserOpenId(user, appAccessKey.getOpenid(), appId);
     ...
     * @return
     */
    @RequestMapping("/ABregister")
    public @ResponseBody
    Map<String, Object> ABregister(@RequestBody UserInfo userInfo) {
        String unionid1 = userInfo.getUnionid();
        UserResult userByUnionid = userService.getUserByUnionid(unionid1);
        if (userByUnionid==null) {
            String nickName = userInfo.getNickname();
            //用户的性别，值为1时是男性，值为2时是女性，值为0时是未知
            int gender = userInfo.getGender();
            String city = userInfo.getCity();//城市
            String province = userInfo.getProvince();//省份
            String country = userInfo.getCountry();//国家
            String headimgurl = userInfo.getHeadimgurl();//头像地址
            String unionid = userInfo.getUnionid();//唯一ID
            String appId = userInfo.getAppid();//appId
            String openId = userInfo.getOpenid();//app内唯一ID
            UserResult ur = null;
            ...
            //生成token
            Token token = tokenManager.createToken(userByUnionid.getUuid());
    ...
    public @ResponseBody
    ApiResult register(final String encryptedData, final String sessionKey, final String iv) {
        ApiResult result = this.buildAjaxResponse(new IWebResponseHandler() {
            @Override
            public Object execute() throws Exception {
    ...
    public @ResponseBody
    ApiResult logintest(final String openid) {
        ApiResult result = this.buildAjaxResponse(new IWebResponseHandler() {
            @Override
            public Object execute() throws Exception {
...
//					userService.addOrUpdateUserOpenId(user, appAccessKey.getOpenid(), appId);
    ...
    public @ResponseBody
    ApiResult bindhisuser(final String openid, final String unionid) {
    ...
    public @ResponseBody
    ApiResult transmit(final String userId, final String newUserId, final String token) {
        ApiResult result = this.buildMobileAuthAjaxResponse(new IWebAuthResponseHandler() {
    ...
    public @ResponseBody
    ApiResult video(final String userId) {
        ApiResult result = this.buildMobileAuthAjaxResponse(new IWebAuthResponseHandler() {
            @Override
            public Object execute(String uuid) throws Exception {
    ...
    public @ResponseBody
    String APPLoginTest(HttpServletRequest request, String map2) throws IOException {
...
//	}
    ...
    public @ResponseBody
    String useCoupon(final String couponno) {
